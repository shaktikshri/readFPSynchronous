...
void PrintErrorMsg(int errorno)
{
    switch(errorno)
    {
		...
		// Absolute crap, only the Divine knows where they got these constants from!!
		/*
...
void MFS100AutoCaptureCallBack(unsigned char* Frame)
{
...
repeat:
    printf("Place finger and press enter to capture first frame\n");
    ret = MFS100AutoCapture(device,10000,60,FinalFrame1,BitmapData1,Iso_19794_2_Template1,&ISOTemplateLength1,Ansi_378_2004_Template1,&ANSITemplateLength1,&Quality1,&NFIQ1,MFS100AutoCaptureCallBack);
    if(ret!=0)
    {
        PrintErrorMsg(ret);
        goto err;
    ...
    printf("Place finger and press any key and enter to capture second frame\n");
    getkey();
    ret = MFS100AutoCapture(device,10000,60,FinalFrame2,BitmapData2,Iso_19794_2_Template2,&ISOTemplateLength2,Ansi_378_2004_Template2,&ANSITemplateLength2,&Quality2,&NFIQ2,MFS100AutoCaptureCallBack);
    if(ret!=0)
    {
        PrintErrorMsg(ret);
        goto err;
...
int main(int argc, char** argv) {
    ...
    ret=MFS100DeviceConnected(device);
    if(ret!=0)
    {
        if(ret!=0)PrintErrorMsg(ret);
        goto err;
